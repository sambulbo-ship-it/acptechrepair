...
FOR EACH ROW
EXECUTE FUNCTION public.update_updated_at_column();-- Enable realtime for all workspace tables
ALTER PUBLICATION supabase_realtime ADD TABLE public.machines;
ALTER PUBLICATION supabase_realtime ADD TABLE public.diagnostic_entries;
ALTER PUBLICATION supabase_realtime ADD TABLE public.team_members;-- Table pour l'historique des scans
...
ALTER PUBLICATION supabase_realtime ADD TABLE public.scan_history;-- Add workspace branding columns
...
USING (user_id = auth.uid());-- Create app_admins table for global app administrators
...
ALTER PUBLICATION supabase_realtime ADD TABLE public.maintenance_schedules;-- Fix 1: Create a function for admins to get workspace invite codes
...
CREATE INDEX idx_repair_locations_workspace_id ON public.repair_locations(workspace_id);-- Table for AI conversation history per user
...
FOR EACH ROW
EXECUTE FUNCTION public.update_updated_at_column();-- Create RPC function to find workspace by invite code without requiring membership
...
GRANT EXECUTE ON FUNCTION public.find_workspace_by_invite_code(TEXT) TO authenticated;-- Create table for custom workspace brands
...
CREATE INDEX idx_workspace_brands_workspace_category ON public.workspace_brands(workspace_id, category);-- Add new machine statuses for rental/sale
...
ADD COLUMN IF NOT EXISTS invoice_url text;-- Fix security issues identified in the scan
...
-- - external_repairs: workspace member access is correct for business operations-- Create quote_requests table for storing client quote requests
...
CREATE INDEX idx_quote_requests_created_at ON public.quote_requests(created_at DESC);-- Fix security issue: quote_requests should only allow INSERT via service role (edge functions)
...
FOR INSERT
TO anon
WITH CHECK (false);-- Fix: Restrict quote_requests SELECT access to workspace admins only
...
FOR SELECT
TO authenticated
USING (created_by = auth.uid());-- Fix: Restrict repair_service_providers to workspace members only
...
ALTER VIEW public.workspaces_public SET (security_invoker = true);-- Simplify: the OR condition was redundant, just use workspace membership
